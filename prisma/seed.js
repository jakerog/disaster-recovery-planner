...
async function main() {
  ...
  // Cleanup legacy admin if exists
  await prisma.resource.deleteMany({
    where: { email: 'admin@sentinel.com' }
  }).catch(() => {});

  if (existingExercise) {
    console.log('Operational data detected. Synchronizing security identities...')
  } else {
    console.log('Initializing fresh recovery environment...')
  ...
  console.log('Sentinel Security Protocol: Synchronization Complete')
}

main()
  .catch(e => {
    console.error('Security Synchronization Fault:', e);
    process.exit(1);
  })

...
export default function AvailabilityForm({ phases, resourceId }: { phases: PhaseWithRelations[], resourceId: string }) {
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-12">
      {phases.map((phase) => (
        <div key={phase.id} className="clean-inset p-8 border border-white/50 space-y-8">
          <div className="flex justify-between items-start">
             ...
             <div className="flex gap-4">
                <label className="flex items-center gap-3 cursor-pointer group">
                  ...
                  <div className="w-10 h-10 clean-card rounded-xl flex items-center justify-center text-gray-300 peer-checked:text-green-500 peer-checked:scale-110 transition-all border-none">
                ...
                <label className="flex items-center gap-3 cursor-pointer group">
                  ...
                  <div className="w-10 h-10 clean-card rounded-xl flex items-center justify-center text-gray-300 peer-checked:text-red-500 peer-checked:scale-110 transition-all border-none">
                    <XCircle size={24} />
                  </div>
          ...
          <div className="pt-2">
            <textarea
              ...
              className="w-full bg-white/40 border-none clean-inset p-4 text-xs font-bold text-gray-700 placeholder:text-gray-300 focus:outline-none focus:ring-1 focus:ring-blue-500/30 rounded-2xl min-h-[100px]"
            />
          </div>
        </div>
      ))}
      ...
      <button
        type="submit"
        disabled={loading}
        className="w-full clean-button text-white p-6 rounded-3xl font-black uppercase tracking-[0.5em] text-[12px] shadow-[0_20px_50px_rgba(0,0,0,0.3)] hover:scale-[1.01] active:scale-[0.98] transition-all"
      >
